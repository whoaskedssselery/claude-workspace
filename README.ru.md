...
Если `pnpm add -g claude-workspace` без версии вдруг ставит неожиданно старую версию — это pnpm
отдаёт устаревший локальный кэш метаданных, а не проблема пакета. Форсируй свежий резолв явной
версией: `pnpm add -g claude-workspace@latest`.
...
- `.claude/skills/` — скиллы выбранного пресета, применяются автоматически, вручную вызывать не
  нужно
...
пакет, с однострочным описанием каждого — каталог достаточно большой, проще посмотреть список, чем
листать репозиторий. Опечатка в имени не проходит молча — предлагается ближайшее совпадение:
...
Несколько имён в каталоге (`impeccable`, `superpowers`, `taste`, `ui-ux-pro-max`) — это не файлы
скиллов, а отдельные инструменты со своим установщиком или маркетплейсом плагинов. `init` никогда
не ставит их автоматически — по умолчанию просто печатает команду установки. Добавь `--with=<name>`,
чтобы реально запустить установщик этого инструмента, или `--with-external`, чтобы поставить сразу
все внешние инструменты, которые перечисляет выбранный пресет.

Это отдельная механика от [`add <url>`](#добавление-скилла-из-любого-репозитория) ниже: внешние
инструменты — полноценные отдельные проекты, а `add` просто скачивает файл скилла в
`.claude/skills/`.
...
→ **дополнительные скиллы** (чекбокс-список: скиллы под технологии, подходящие варианты формата,
внешние инструменты — у каждого пункта подсказка из его `SKILL.md`) → подтверждение и установка.
...
skills/     каталог скиллов, по одной папке на скилл: skills/<домен>/<имя>/SKILL.md
...
Код самого пакета — MIT (см. [LICENSE](LICENSE)), но часть скиллов в `skills/` — vendored из
других MIT-проектов, а не написана для этого репозитория. Каждый сохраняет свою исходную лицензию
и авторство; здесь ничто не релицензируется и не выдаётся за собственную разработку.
...
разработка этого проекта, не vendored.

Внешние инструменты (`impeccable`, `superpowers`, `taste`, `ui-ux-pro-max` — см. [Внешние
инструменты](#внешние-инструменты)) вообще не vendored: `init`/`add` только запускают их
собственный установщик, их код живёт и остаётся в их собственных репозиториях.
