...
Или без имени пресета, в интерактивном терминале — запустится пошаговый мастер (выбор пресета,
выбор дополнительных скиллов, подтверждение) — см. [Интерактивный мастер](#интерактивный-мастер):

```bash
npx claude-workspace init
```

Точно так же работает при глобальной установке, если не хочется каждый раз писать `npx`:
...
`claude-workspace init` без имени пресета, запущенный в интерактивном терминале, проводит через те
же решения пошагово, без необходимости указывать флаги заранее:
...
3. **Дополнительные скиллы** — чекбокс-список (пробел — переключить, enter — подтвердить), разбитый
   на domain skills, релевантные варианты формата (только `assignment-defend` для `assignment`,
   только `spike` для `project` — никогда оба сразу) и external tools.
4. Если выбраны external tools: **установить их прямо сейчас или только показать команды?**
5. **Подтверждение** — сводка того, что именно будет создано, затем выполнение (или отмена).

При создании кастомного пресета вместо этого показывается весь каталог (все папки `skills/` плюс
external tools), а в конце спрашивается, **сохранить ли пресет глобально** в
`~/.claude-workspace/presets/<name>.yaml` — если да, `claude-workspace init <name>` дальше работает
неинтерактивно точно так же, как встроенный пресет (см. [Кастомные пресеты](#кастомные-пресеты)).

Мастеру нужен настоящий TTY (raw-mode ввод с клавиатуры) — запуск из скрипта, CI или с stdin из
файла аккуратно откатывается к понятной ошибке с просьбой указать имя пресета явно. Всё это без
зависимостей: никакого `inquirer`, только встроенный `readline` Node.

## Кастомные пресеты

Пресеты не ограничены пятью встроенными. `~/.claude-workspace/presets/<name>.yaml` — созданные через
путь "создать свой пресет" в мастере, либо написанные вручную в том же формате, что встроенные в
[`presets/`](presets/) — подхватываются `init`/`list` точно так же, как встроенный пресет. Встроенные
пресеты всегда имеют приоритет при совпадении имени, так что кастомный пресет не может случайно
перекрыть `learning`, `project` и т.д.
...
— нужен `sync`), либо **missing**. Также проверяет наличие `CLAUDE.md` и блока в `.gitignore`.
...
`init --force` перезаписывает существующий `CLAUDE.md` вместо того, чтобы оставить его нетронутым.
