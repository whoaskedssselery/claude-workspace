...
- `.claude-workspace/hide.yaml` — плоский список путей в корне проекта (начиная с самих `.claude` и
  `CLAUDE.md`), которые [`hide`](#временно-спрятать-воркспейс) умеет убирать из проекта прямо перед
  коммитом

`.claude/skills/`, `.claude/workspace.yaml`, `CLAUDE.md` и `.claude-workspace/hide.yaml` — всё это
коммитится и расшаривается на команду, это и есть смысл инструмента. Ничего не игнорируется через
git: если что-то из добавленного claude-workspace не должно попасть в коммит — сначала сделайте
`hide` (см. ниже).
...
нужен `sync`) или **missing** — плюс совпадает ли записанная версия тулкита с текущей и настроен
ли корректно `CLAUDE.md`.
...
```bash
claude-workspace hide     # спрятать всё, что перечислено в .claude-workspace/hide.yaml
claude-workspace unhide   # вернуть всё обратно, точно как было
```

Ничего из того, что claude-workspace добавляет в проект, не игнорируется через git — `.claude/`,
`CLAUDE.md`, всё это коммитится. Когда что-то из этого не должно попасть в конкретный коммит
(скриншеринг, чистый `git diff`, передача проекта тому, кто пока не должен это видеть) — вместо
того чтобы навсегда исключать это из git, сначала сделайте `hide`.

`hide` перемещает каждый путь, перечисленный в собственном `.claude-workspace/hide.yaml` проекта, в
тайник **полностью за пределами проекта** (в `~/.claude-workspace/hidden/`, по ключу от пути
проекта), так что проект выглядит так, будто `init` никогда не запускался, и в дереве проекта (или
в `git status`) не остаётся вообще ничего — запись в `.gitignore` прячет папку только от git, а не
от IDE, поэтому тайник и не живёт внутри проекта.

`hide.yaml` — единственное, что проверяет `hide`: никакой отдельной обработки для
`.claude/skills/`, `workspace.yaml` или сгенерированного блока в `CLAUDE.md` больше нигде нет:
...
`init` сразу прописывает туда сами `.claude` и `CLAUDE.md`, плюс всё, что известно как создаваемое
только что установленным скиллом/инструментом сверх них — это объявлено прямо там, где определён
сам скилл/инструмент: во фронтматтере `SKILL.md` core- или format-скилла (`creates:`), или рядом с
записью в `REMOTE_SKILLS`/`EXTERNAL_TOOLS` (например `.impeccable/` от impeccable, `.codegraph/` от
codegraph, `product-facts.md`/`brand-spec.md` от claude-design). `add` и `sync` поддерживают список
актуальным так же — например, инструмент, добавленный вручную в `workspace.yaml` и подхваченный
`sync`, тоже получит свои пути.

Для всего, о чём claude-workspace не может узнать сам — инструмент, настроенный вручную, личная
заметка, локальный файл, который просто не должен быть виден во время скриншеринга — впишите это
сами, в тот же файл:
...
Коммитится как кастомный пресет, так что после клонирования у всей команды будет одинаковое
поведение `hide`. Каждый путь — относительно корня проекта; всё, что вело бы за пределы проекта
(`..`, абсолютный путь), пропускается с предупреждением, а не перемещается.

Работает одинаково независимо от того, установлен ли сам `claude-workspace` глобально или только в
этом одном проекте.
...
scripts/lib/manifest.js  установленный воркспейс проекта: workspace.yaml, CLAUDE.md, hide.yaml
...
каждом push и PR). Покрывает парсер YAML, подсказки при опечатках,
`init`/`sync`/`doctor`/`add`/`remove`/`hide`/`unhide` end-to-end во временной директории на
реальных пресетах, свои пресеты и неинтерактивную логику визарда. Raw-режим клавиатуры визарда
требует настоящий TTY и проверяется вручную, а не в CI.
