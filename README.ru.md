...
Одна команда даёт Claude Code нужные скиллы, workflow и инструкции по проекту — воспроизводимо и
расшарено на всю команду, так же как `package.json` фиксирует зависимости. Подбор идёт под *способ
работы* (обучение, повседневная работа, охота на баг, редизайн...), а не под конкретный стек.
...
- [Концепции](#концепции)
- [Пресеты](#пресеты)
- [Добавление всего остального через `--with=`](#добавление-всего-остального-через---with)
- [Добавление скилла из любого репозитория](#добавление-скилла-из-любого-репозитория)
- [Управление уже настроенным воркспейсом](#управление-уже-настроенным-воркспейсом)
- [Временно спрятать воркспейс](#временно-спрятать-воркспейс)
- [Полный список команд](#полный-список-команд)
- [Интерактивный визард](#интерактивный-визард)
- [Свои пресеты](#свои-пресеты)
...
`init` — команда для настройки с нуля, один раз: она откажется работать на уже настроенном проекте
(используй `add`/`remove`/`sync`) или на спрятанном сейчас проекте (сначала
[`unhide`](#временно-спрятать-воркспейс)). У визарда та же защита.
...
- `.claude/hide.yaml` — плоский список путей в корне проекта (начиная с самих `.claude` и
  `CLAUDE.md`), которые [`hide`](#временно-спрятать-воркспейс) умеет убирать из проекта прямо перед
  коммитом

`.claude/skills/`, `.claude/workspace.yaml`, `CLAUDE.md` и `.claude/hide.yaml` — всё это
...
Устанавливаются вещи четырёх видов, и каталог держит их отдельно, потому что у каждого свой способ
установки:

- **preset (пресет)** — способ работы, ставится единым набором (`learning`, `debug`, ...)
- **skill (скилл)** — одна способность, к которой Claude Code обращается, когда задача с ней
  совпадает; либо вендорится прямо в этом репозитории, либо подтягивается по требованию из
  репозитория автора
- **external tool (внешний инструмент)** — способность со своим установщиком или plugin
  marketplace, не файл скилла (`impeccable`, `superpowers`, ...) — `init`/`add` только запускают
  его установщик
- **format (формат)** — небольшой опциональный вариант-скилл поверх пресета (`spike`,
  `assignment-defend`)

`--with=` (ниже) добавляет к пресету любое из последних трёх. Как именно каждое из них
подтягивается — см. [Откуда реально берутся файлы скилла](#откуда-реально-берутся-файлы-скилла).
...
```bash
claude-workspace hide     # спрятать всё, что перечислено в .claude/hide.yaml
...
того чтобы навсегда исключать это из git, сначала сделайте `hide`: `hide`, затем обычный коммит и
пуш, затем `unhide`, чтобы вернуть всё локально — в запушенной истории для этого коммита не
остаётся ни следа claude-workspace, и в рабочей директории тоже ничего не остаётся.

`hide` перемещает каждый путь, перечисленный в собственном `.claude/hide.yaml` проекта, в тайник
...
Он намеренно лежит по пути `.claude/hide.yaml` — внутри той самой папки, которую по большей части и
описывает: `.claude` сам по себе один из путей, которые прячутся как единое целое, так что
`hide.yaml` вместе с остальным `.claude/` уезжает в тайник и возвращается обратно при `unhide`, как
и всё остальное из списка. В корне проекта в любом случае не остаётся ничего, кроме `.claude/` и
`CLAUDE.md`.
...
`sync`, тоже получит свои пути. А `remove` наоборот убирает пути удалённого имени из списка — если
только их не объявляет ещё какое-то из установленных сейчас имён.
...
Коммитится вместе с остальным `.claude/`, так что после клонирования у всей команды будет
