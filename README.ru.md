...
`.claude/workspace.yaml`, сгенерированный блок в `CLAUDE.md`, и собственные папки внешних
инструментов, о которых известно (например `.impeccable/` от impeccable) — в тайник **полностью
...
тайник и не живёт внутри проекта. Полезно для скриншеринга, чистого `git diff` или передачи
проекта тому, кто не должен это видеть, — при этом ничего не теряется. `.gitignore` не трогается
вообще, и всё работает одинаково независимо от того, установлен ли сам `claude-workspace`
глобально или только в этом одном проекте.
